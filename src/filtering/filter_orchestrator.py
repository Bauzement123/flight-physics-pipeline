...
def extract_airports_from_ranks(route_summary_path: str, ranks: list) -> pd.DataFrame:
    ...
    """
    logging.info(f"Extracting route corridors from summary for ranks: {ranks}")
    df_summary = load_route_summary(route_summary_path)
    
    if df_summary.empty:
        logging.warning("No records found in RouteSummary.")
        return pd.DataFrame()
...
def filtered_lists_from_ranks(airports_df: pd.DataFrame, master_file_path: str, output_dir: str):
    ...
    if not master_path.exists():
        logging.error(f"Master flight registry not found at: {master_path}")
        return

    # Determine file format & load database
    is_parquet = master_path.suffix.lower() == '.parquet'
    logging.info(f"Reading master database: {master_path.name} (Format: {'Parquet' if is_parquet else 'CSV'})")
    ...
    try:
        ...
        logging.error(f"Failed to load master database: {e}")
        return
    ...
    if not dep_col or not arr_col:
        logging.error(f"Could not identify airport columns in master database. Columns: {list(master_df.columns)}")
        return

    logging.info(f"Using database mapping: Departure='{dep_col}', Arrival='{arr_col}'")
    ...
    for _, row in airports_df.iterrows():
        ...
        if target_path.exists():
            logging.info(f"Skipping Rank {rank} ({dep} -> {arr}): {target_filename} already exists.")
            continue

        logging.info(f"Processing Rank {rank}: Slicing corridor for {dep} -> {arr}...")

        # Filter target routes
        filtered_flights = master_df[(master_df[dep_col] == dep) & (master_df[arr_col] == arr)].copy()

        if filtered_flights.empty:
            logging.warning(f"  -> No flights found in registry for {dep} -> {arr}.")
            continue

        # Save to Parquet
        try:
            filtered_flights.to_parquet(target_path, index=False)
            logging.info(f"  -> Success: Saved {len(filtered_flights):,} flights to {target_path}")
        except Exception as e:
