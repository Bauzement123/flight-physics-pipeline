...
- `raw/` — raw OpenSky waypoints (`*_raw.parquet`)
- `clean/` — EKF-smoothed SI trajectories (`*_clean_si.parquet`)
- `fetch_runs/` — per-run manifest JSON files

**Run tracking**: Each batch fetch run generates a deterministic run ID via `generate_dataset_name()` from `src.common.utils`. The format is:
`ranks_[ranks_spec]_strat_[strategy]_val_[val]_seed_[seed]_format_[format]_start_[start]_end_[end]_[hash_suffix]`

This ID is logged as a progress milestone and embedded in run manifest JSON files — it is **not** used as a directory name.

*   **Ranks Specification (`[ranks_spec]`)**: Range uses `to` (e.g., `ranks_1to5`), list uses hyphens (e.g., `ranks_1-5`).
*   **Hash Suffix**: A deterministic 6-character MD5 checksum of all CLI parameters generated via `src.common.utils.generate_dataset_name()`.
...
| `*_corridor_c[ID].parquet` | Temporal-gridded DTW trajectory route centroids (medoids) generated by `corridor_clustering_worker.py` and stored in `data/corridor_paths/`. Indexed by `GLOBAL_CORRIDOR_MODEL_REGISTRY`. | Parquet |
...
* **Global Configuration & Path Centralization**: Shared variables (e.g., vertical levels, weather variables lists, grid resolutions) and all directory/file path configurations (such as global registry `.parquet` databases, default logs, and route summary pickles/CSVs) must be centrally declared in `src/common/config.py` and imported rather than constructed or hardcoded locally. 
  ...
  - *Symlink-Safe Registry Path Computation*: Registry `file_path` entries must be written as POSIX-style relative paths (e.g., `data/results/corridor_simulations/EDDF-EGLL/file.parquet`). **Never use `path.resolve().relative_to(BASE_DIR)` to compute these.** `Path.resolve()` follows OS-level symlinks and raises a `ValueError` when `data/` is a symlink pointing to a different filesystem (e.g., an NFS mount on the Linux VM). Use `to_registry_path(path)` from `src.common.utils` instead — it calls `os.path.relpath()`, which is pure string arithmetic and works correctly across Windows (no symlinks), macOS, and Linux NFS mounts alike.
