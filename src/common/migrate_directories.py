...
def migrate():
    logger.info(f"Scanning trajectories directory: {TRAJECTORIES_DIR}")
    if not TRAJECTORIES_DIR.exists():
        logger.error("Trajectories directory does not exist.")
        return
    ...
    for cohort_dir in TRAJECTORIES_DIR.iterdir():
        ...
        logger.info(f"Checking cohort directory: {cohort_dir.name}")
        ...
        if not raw_files and not clean_files:
            logger.info(f"  -> No flat parquet files found in {cohort_dir.name}. Skipping.")
            continue
        ...
        if raw_files:
            ...
            logger.info(f"  -> Moving {len(raw_files)} raw files to {raw_dir.relative_to(BASE_DIR)}")
            for f in raw_files:
                dest = raw_dir / f.name
        ...
        if clean_files:
            ...
            logger.info(f"  -> Moving {len(clean_files)} clean files to {clean_dir.relative_to(BASE_DIR)}")
            for f in clean_files:
                dest = clean_dir / f.name
                shutil.move(str(f), str(dest))
                
    # Rebuild the global registry to ensure all cache pointers map to the new "raw/" path structure
    logger.info("Rebuilding global trajectory registry...")
