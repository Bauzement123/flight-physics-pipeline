...
def label_flight_phase(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    res = df.copy()
    if res.empty or any(col not in res.columns for col in ['baroaltitude', 'velocity', 'vertrate', 'time']):
        if 'flight_phase' not in res.columns:
            res['flight_phase'] = None
        return res
    icao24 = str(res.get('icao24', pd.Series([None])).iloc[0]) if 'icao24' in res.columns and not res.empty else 'UNK'
    typecode = str(res.get('typecode', pd.Series([None])).iloc[0]) if 'typecode' in res.columns and not res.empty else None
    ...
    try:
        ...
        df_sorted = res.sort_values('time')
        ...
        res.loc[df_sorted.index, 'flight_phase'] = fp.phaselabel()
...
_trino_client: Any = None


def _get_trino_client() -> Any:
    """Lazy initializer factory for pyopensky Trino client."""
...
def resolve_flight(
    ...
    df = _resolve_from_trino(record, trino_client)
    if df is not None:
...
def update_raw_concat(concat_path: Path, new_dfs: list[pd.DataFrame], df_old: pd.DataFrame | None = None) -> None:
...
def _run_resolution_loop(
    ...
    cached_flights: dict[str, str],
    concat_df: pd.DataFrame | None,
) -> tuple[list[FlightFetchOutcome], list[pd.DataFrame], list[dict[str, str]], list[str], int, int, int]:
    """Executes resolution loop across records."""
    ...
    for idx, rec in enumerate(records, 1):
        ...
        outcome, df = resolve_flight(rec, cached_flights, concat_df)
    ...
    return outcomes, new_dfs, new_reg_entries, failed_fids, reg_hits, concat_rec, trino_f
...
def fetch_trajectories(
    ...
    outcomes, new_dfs, new_reg_entries, failed_fids, reg_hits, concat_rec, trino_f = _run_resolution_loop(
        records, cached_flights, concat_df
    )
    ...
    if update_concat and (concat_rec > 0 or trino_f > 0 or not concat_path.exists()):
        update_raw_concat(concat_path, new_dfs, df_old=concat_df)
    ...
    import gc
    del outcomes, new_dfs, cached_flights, concat_df
    gc.collect()
    try:
        import pyarrow
